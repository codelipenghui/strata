...
(* failover and lost-ack retries (§4 of                                       *)
(* strata-tech-design.md, and the review's idempotency finding).              *)
(*                                                                         *)
(* The design (§4) CLAIMS every mutating client request carries an            *)
(* operation id, and that the leader records applied operation ids in durable *)
(* state (log/snapshot) so retries are idempotent across failover. §4         *)
(* lists an "idempotency table watermark"; the ACTIVE leader barrier          *)
(* lists "restore retained idempotency state"; the durability rules make      *)
...
(*       the table on -- the §4 "idempotency retention is a lower             *)
(*       bound for safe truncation" rule, made teeth.                         *)
(*                                                                         *)
(* Expected results (see the .cfg):                                           *)
