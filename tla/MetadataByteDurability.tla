...
(***************************************************************************)
(* Byte-level durability of the namespace metadata log (§4.2 of              *)
(* strata-tech-design.md). The namespace metadata log is an ordered, durable *)
(* log of metadata mutations stored as a replicated Strata file, so its      *)
(* bytes use the same chunk machinery as user data.                          *)
(*                                                                         *)
(* This model isolates two policy/integrity dimensions of that path:          *)
(*                                                                         *)
(*   FINDING A (availability/durability of the policy).  Metadata-log files  *)
(*   use the configured Strata write policy; the current default is RF=3,    *)
(*   W=2 rather than write-to-all. The CONSTANT toggle WriteAll compares an  *)
(*   all-RF policy with a quorum width W. The                                 *)
...
(*                                                                         *)
(*   INTEGRITY DIMENSION. Current recovery verifies open-tail ledger payload  *)
(*   CRCs before sealing, and sealed reads verify CRC_RANGES before returning *)
(*   bytes (with replica fallback above the store). UseContentCRC=TRUE        *)
(*   selects that current verified behavior. FALSE is a deliberate negative  *)
(*   control showing what a length/common-prefix-only reconstruction would    *)
(*   permit; it is not a description of current code.                        *)
(*                                                                         *)
...
(*                                                                         *)
(* The default cfg models current RF=3/W=2 fail-stop durability with           *)
(* ReadW=2 and MaxCorrupt=0 (W+ReadW>RF). MetadataByteIntegrity.cfg           *)
(* separately models one corrupt copy with all three reachable candidates    *)
(* and UseContentCRC=TRUE. A simultaneous distinct replica failure plus       *)
(* corruption exceeds what W=2 alone can guarantee and is not claimed here.   *)
(* TEETH: UseContentCRC=FALSE in the integrity scenario violates              *)
(* NoSilentCorruption; a too-small quorum violates NoAckedLoss.                *)
(***************************************************************************)
...
(*    a wrong value.                                                          *)
(*                                                                         *)
(*  UseContentCRC = FALSE: the negative-control common-prefix path. It trusts *)
