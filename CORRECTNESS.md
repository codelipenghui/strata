...
| Metadata commit precedes physical writes for live chunks. | `RecoveryCatchUpTest` and `RepairAndRetentionTest` exercise recovery and orphan handling under the commit-before-write contract. The currently automated `ChunkReplication.tla` model does not model metadata creation or orphan confirmation, so it is not cited as evidence for this invariant. |
| A superseded namespace owner cannot authorize physical orphan deletion. | `NamespaceLogBackendReacquireOnFenceTest.staleOwnerCannotAuthorizeOrphanButCurrentOwnerConfirmsReferencedReplica` creates state only in successor B and proves stale cached A fails closed without reacquiring. `OrphanGcTest.skipsStaleOwnerEpochAndTrustsLaterFreshOwnerReference` puts stale A first and proves the node continues to B and retains the referenced chunk. `DataNodeWireTest.ownerEpochFloorSurvivesRestartOnTheSameVolume` proves a consensus-validated owner floor survives data-node restart. `ShardedClusterEndToEndTest.orphanGcWalksPastNonOwnerAfterShardedControllerRestart` composes the namespace-log authority path, endpoint walk, controller replacement, and physical reclamation over real sockets and disks. |
...
| Repair never promotes corrupt, partial, or stale replicas. | `RepairAndRetentionTest`, `RepairCoordinatorTest`, `RepairReliabilityTest`, deterministic stress/fault artifacts, and process-crash repair cases cover copy verification, replicate/delete command retry, stale owner-pull verification and liveness snapshots, scrubbed sealed-replica bit rot, lost completion acknowledgements, and convergence back to full readable RF after a killed data node rejoins. |
...
| A healed storage-network partition cannot preserve a stale open replica. | `ChaosTest.healedReplicaPartitionRepairsStaleOpenCopyToSealedRf` partitions all client data traffic to one required RF=4 replica, continues writes and seal through AQ=3, heals the link, then requires repair to replace the stale local copy and converge to four byte-identical sealed replicas. `ChaosTest.processDataNodePartitionHealsAndRepairsStaleOpenCopy` repeats the same property with data nodes running as child JVMs and only the data endpoint routed through Toxiproxy. |
| Stale owner-pull verification cannot erase the last healthy replica. | `RepairCoordinatorTest.lastLiveGuardCountsDrainingPeerAsSurvivor`, `verifyPassReReadsPersistedLivenessBeforeDroppingLastReplica`, and `verifyPassCombinesFreshLivenessWithAlreadyDroppedReplicas` prove a verification pass rechecks current liveness and accounts for replicas already dropped in the same pass before applying a destructive descriptor change. |
| A suspect sealed descriptor cannot trigger irreversible verify-lane evidence loss. | `RepairCoordinatorTest.verifyPassBreaksWhenNoReplicaMatchesDescriptor` preserves every sealed replica when no returned length/CRC matches the descriptor, while `verifyPassBreaksBeforeResealingUnanimousOpenMismatch` proves a wrong descriptor cannot truncate unanimously mismatching OPEN evidence. `staleVerifyVerdictCannotQuarantineTargetWhileRepairCommitIsPublishing` serializes repair commit with stale-verdict application. `ChunkStoreTest.quarantineAtomicallyRenamesEvidenceAndCountsItAcrossRestart` proves quarantine preserves the complete chunk image and sidecars under non-live names, forces the shard directory, and keeps retained bytes in capacity accounting across restart. `DataNodeWireTest.quarantineChunksPreservesEvidenceAndCapacityAcrossNodeRestart` proves the positive-owner-epoch wire lane removes only the live name while preserving the same evidence and accounting on a restarted node; `quarantineRechecksOwnerEpochAfterWaitingForDeletePermit` proves a request made stale while waiting for disk QoS is fenced before mutation. |
...
| Metadata backend semantics are stable across implementations. | `MetadataStoreConformanceTest` runs against `ZkMetadataStoreConformanceTest`, `NamespaceLogMetadataStoreConformanceTest`, and `InMemoryMetadataStoreConformanceTest`, covering file/path/node CRUD, namespace scoping, list visibility, CAS wins/losses, failed duplicate path creation without orphan records, replacement-safe final deletes, idempotent missing deletes, store-handle close/reopen preservation of state and versions, and stale handles being unable to disturb a replacement. Deleted file ids remain reserved while their tombstones exist; the explicit sweep contract permits reuse after the tombstone is reaped. `NamespaceLogMetadataStoreConformanceTest.facadeOwnershipClosesTheProcessBackendExactlyOnce` separately locks the shared-backend ownership boundary. |
...
current compatibility/conformance, embedded fault, Docker/Toxiproxy chaos, and bounded-soak gates,
then uploads test reports, process-crash logs, metadata-process logs, and replayable correctness artifacts.
`.github/workflows/tla.yml` is triggered when a TLA+ spec or its tooling changes, but the current
`scripts/tlc.sh` gate checks only `ChunkReplication.tla`. The metadata models are not yet automated.

`.github/workflows/correctness.yml` is the scheduled/manual deep-confidence workflow. It runs the
default, compatibility, embedded-fault, current single-model TLA+, Docker/Toxiproxy chaos, and larger bounded-soak jobs.
Manual runs can toggle individual gates and supply `compat_versions` to add the SCP released-artifact
...
- Metrics, Prometheus exposition, and Grafana dashboards are implemented; production alert policy and
  paging thresholds remain deferred.
