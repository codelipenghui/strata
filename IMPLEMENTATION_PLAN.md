...
- **v0 protocol additions** (additive per tech design §10.6; fold back into the design doc at finalize):
  ...
  - Client↔metadata over SCP, opcode range 0x02xx: `CREATE_FILE` 0x0201, `CREATE_CHUNK` 0x0202, `SEAL_CHUNK_META` 0x0203, `LOOKUP_FILE` 0x0204, `DELETE_FILES` 0x0205, `SEAL_FILE` 0x0206, `ABORT_CHUNK_META` 0x0207, `LOOKUP_PATH` 0x0208, `ALLOCATE_WRITER_EPOCH` 0x0209, `CONFIRM_ORPHAN` 0x020B (0x020A remains unassigned). (v1 moves broker-facing APIs to Kafka RPC; range stays for tools.)
